...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById('deleteCategoryIcon').addEventListener('click', deleteCurrentCategory);
...
window.generateTasks = async function generateTasks() {
	const description = document
		.getElementById("aiActivityDescription")
		.value.trim();
	if (!description) {
	...
	try {
				...
				messages: [
					{
						role: "system",
						content:
							"You are a helpful assistant that breaks down activities into task lists.",
					},
					{
						role: "user",
		...
		if (response.ok) {
			...
			const [listName, ...taskDescriptions] = result
				.split("\n")
